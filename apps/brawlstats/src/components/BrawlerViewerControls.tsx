...
export function BrawlerViewerControls({ playing, skinOptions = [], animationOptions = [], selectedSkin, selectedAnimation, onSelectSkin, onSelectAnimation, face, outline, faceEnabled = false, outlineEnabled = false, onTogglePlaying, onToggleFace, onToggleOutline }: BrawlerViewerControlsProps) {
  ...
  return (
    <div className="pointer-events-auto absolute inset-x-3 bottom-3 z-20 flex items-center gap-2 rounded-lg bg-background/80 p-2 text-xs backdrop-blur">
      ...
      {skinOptions.length > 1 ? <select aria-label="Skin" value={selectedSkin} onChange={(event) => onSelectSkin?.(event.target.value)}>{skinOptions.map((skin) => <option key={skin.id} value={skin.id}>{skin.label}</option>)}</select> : null}
      {animationOptions.length > 0 ? <select aria-label="Animation" value={selectedAnimation} onChange={(event) => onSelectAnimation?.(event.target.value)}>{animationOptions.map((animation) => <option key={animation.key} value={animation.key}>{animation.label}</option>)}</select> : null}
