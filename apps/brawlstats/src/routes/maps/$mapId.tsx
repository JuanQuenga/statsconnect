        ...
        return (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="font-medium">{meta?.name || `${t("player.brawler")} ${row.brawlerId}`}</p>
      ...
      {rows.map((row) => (
        ...
        >
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            <span className="text-primary">{t("maps.winRateText", { rate: formatPercent(row.winRate) })}</span> · {t("maps.games", { count: trophies(row.picks) })}
          </p>
        </Card>
      ))}
    </div>
