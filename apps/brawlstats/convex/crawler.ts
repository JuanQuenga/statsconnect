...
async function fetchJson(ctx: ActionCtx, path: string, endpoint: string): Promise<unknown> {
  ...
  await ctx.runMutation(internal.pipeline.logFetch, {
    endpoint,
...
export const discover = internalAction({
  ...
  handler: async (ctx) => {
    const runId = await ctx.runMutation(internal.pipeline.startRun, { job: "discover" });
    let discovered = 0;
    let added = 0;
    let failures = 0;
    ...
    try {
      ...
      const result = await ctx.runMutation(internal.pipeline.upsertTargets, { targets: unique });
      let directoryRecorded = 0;
      for (let index = 0; index < sightings.length; index += 100) {
        const directory = await ctx.runMutation(internal.players.recordSightings, {
          players: sightings.slice(index, index + 100),
        });
        directoryRecorded += directory.recorded;
      }
      added = result.added;
      await ctx.runMutation(internal.pipeline.bumpCounters, {
        counters: [
...
export const crawl = internalAction({
  ...
  handler: async (ctx) => {
    const runId = await ctx.runMutation(internal.pipeline.startRun, { job: "crawl" });
    ...
    try {
      const targets = await ctx.runMutation(internal.pipeline.claimTargets, {
        limit: batchSize,
        leaseMs: 5 * 60 * 1_000,
      });
      ...
      for (const target of targets) {
        try {
          ...
          if (profile) {
            const snapshot = await ctx.runMutation(internal.players.recordProfile, profile);
          ...
          const newItems = since
            ...
            : allItems;
          const result = await ctx.runMutation(internal.ingest.ingestBattleLogItems, {
            items: newItems,
            focusTag: target.tag,
          });
          fetched += 1;
          battles += result.inserted;
          await ctx.runMutation(internal.pipeline.completeTarget, {
            id: target.id,
            ok: true,
...
export const prune = internalAction({
  ...
  handler: async (ctx) => {
    const runId = await ctx.runMutation(internal.pipeline.startRun, { job: "prune" });
    const now = Date.now();
    let deleted = 0;

    try {
      for (let batch = 0; batch < 20; batch += 1) {
        const result = await ctx.runMutation(internal.pipeline.pruneBatch, {
      ...
      await ctx.runMutation(internal.pipeline.finishRun, { id: runId, ok: false, note });
      throw error;
    }
  },
});
