...
export const search = query({
  ...
  handler: async (ctx, args) => {
    ...
    const [tagMatch, exactMatches, fuzzyMatches] = await Promise.all([
      possibleTag
        ? ctx.db.query("playerDirectory").withIndex("by_tag", (q) => q.eq("tag", possibleTag)).unique()
        : null,
      ctx.db
        .query("playerDirectory")
        ...
        .take(limit),
      raw.length >= 2
        ? ctx.db.query("playerDirectory").withSearchIndex("search_name", (q) => q.search("name", raw)).take(limit)
        : [],
    ]);
...
export const directorySize = query({
  ...
  handler: async (ctx) => {
    const rows = await ctx.db.query("playerDirectory").take(10_001);
