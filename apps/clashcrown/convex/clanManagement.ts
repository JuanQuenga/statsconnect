...
export const prepareObservation = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existing) {
      await ctx.db.patch("trackedClans", existing._id, { nextObservationAt: leaseUntil, lastError: undefined });
      return { shouldObserve: true, lastObservedAt: existing.lastObservedAt ?? null };
    }

    await ctx.db.insert("trackedClans", {
      tag: args.tag,
...
export const recordObservationFailure = internalMutation({
  ...
  returns: v.null(),
  handler: async (ctx, args) => {
    const tracked = await ctx.db.query("trackedClans").withIndex("by_tag", (q) => q.eq("tag", args.tag)).unique();
    if (tracked) {
      await ctx.db.patch("trackedClans", tracked._id, {
...
export const recordObservation = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.insert("clanRosterSnapshots", {
      clanTag,
    ...
    for (const member of members) {
      ...
      await ctx.db.insert("clanMemberSnapshots", {
        clanTag,
        observedAt: args.observedAt,
      ...
      if (!previous) {
        await ctx.db.insert("clanActiveMembers", {
          clanTag,
          memberTag: member.tag,
          name: member.name,
          role: member.role,
        ...
        if (previousSnapshot) {
          await ctx.db.insert("clanManagementEvents", {
            clanTag,
            observedAt: args.observedAt,
            kind: "joined",
            memberTag: member.tag,
      ...
      await ctx.db.patch("clanActiveMembers", previous._id, {
        name: member.name,
        role: member.role,
        trophies: member.trophies,
        trophyChange,
    ...
    for (const previous of activeRows) {
      ...
      await ctx.db.delete("clanActiveMembers", previous._id);
      await ctx.db.insert("clanManagementEvents", {
        clanTag,
        observedAt: args.observedAt,
        kind: "left",
    ...
    for (const week of args.warWeeks.slice(0, MAX_WAR_WEEKS)) {
      for (const member of week.members.slice(0, MAX_MEMBERS)) {
        ...
        if (existing) {
          await ctx.db.patch("clanWarMemberWeeks", existing._id, values);
        } else {
          await ctx.db.insert("clanWarMemberWeeks", {
            clanTag,
    ...
    const tracked = await ctx.db.query("trackedClans").withIndex("by_tag", (q) => q.eq("tag", clanTag)).unique();
    if (tracked) {
      await ctx.db.patch("trackedClans", tracked._id, {
        name: args.clan.name,
        lastObservedAt: args.observedAt,
...
export const dashboard = query({
  ...
  handler: async (ctx, args) => {
    ...
    const tracked = await ctx.db.query("trackedClans").withIndex("by_tag", (q) => q.eq("tag", tag)).unique();
...
export const pruneSnapshots = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const batches = await Promise.all([
      ctx.db.query("clanRosterSnapshots").withIndex("by_observed_at", (q) => q.lt("observedAt", cutoff)).take(100),
      ctx.db.query("clanMemberSnapshots").withIndex("by_observed_at", (q) => q.lt("observedAt", cutoff)).take(300),
      ctx.db.query("clanManagementEvents").withIndex("by_observed_at", (q) => q.lt("observedAt", cutoff)).take(100),
      ctx.db.query("clanWarMemberWeeks").withIndex("by_last_observed_at", (q) => q.lt("lastObservedAt", cutoff)).take(300)
    ]);
