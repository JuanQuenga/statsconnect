...
export const recordLeaderboardSnapshot = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const snapshotId = await ctx.db.insert("leaderboardSnapshots", {
    ...
    if (board) {
      ...
      await ctx.db.insert("leaderboardBoards", {
        ...args.board,
...
const publicLeaderboardSnapshot = v.object({
  id: v.id("leaderboardSnapshots"), observedAt: v.number(), lastObservedAt: v.number(), entryCount: v.number(), baseline: v.boolean()
});
...
export const getLeaderboardSnapshot = query({
  args: {
    ...
    compareToId: v.optional(v.id("leaderboardSnapshots")),
  ...
  handler: async (ctx, args) => {
    ...
    const board = await ctx.db.query("leaderboardBoards").withIndex("by_key", (q) => q.eq("key", snapshot.boardKey)).unique();
    ...
    let previous: Doc<"leaderboardSnapshots"> | null = null;
    let previousByTag = new Map<string, Doc<"leaderboardEntries">>();
    if (args.compareToId) {
...
export const backfillLegacyProfileHistory = internalMutation({
  ...
  handler: async (ctx, args): Promise<{ migrated: number; isDone: boolean; continueCursor: string }> => {
    ...
    if (!page.isDone) {
      await ctx.scheduler.runAfter(0, internal.history.backfillLegacyProfileHistory, {
