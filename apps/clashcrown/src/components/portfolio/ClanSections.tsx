  ...
  return (
    <ArenaHeroFrame className="clan-hero">
      ...
      <div className="clan-summary">
        <Summary icon="/images/ui-icons/trophies.png" value={formatNumber(clan.score)} label={locale === "es" ? "Trofeos del clan" : "Clan Trophies"} />
        <Summary icon="/images/ui-icons/trophies.png" value={formatNumber(clan.requiredTrophies ?? 0)} label={locale === "es" ? "Trofeos requeridos" : "Required Trophies"} />
  ...
  return (
    <section className="clan-progress profile-section">
      ...
      <div className="progress-columns">
        ...
        <ProgressItem icon="/images/ui-icons/trophies.png" label={`${formatNumber(clan.warTrophies)} ${locale === "es" ? "trofeos de guerra" : "war trophies"}`} total={locale === "es" ? "en vivo" : "live"} progress={warProgress} />
      </div>
    </section>
  );
}
          ...
          <tbody>
            {clan.members.map((member, index) => (
              <tr key={member.tag ?? member.name}>
                ...
                <td><Image src="/images/ui-icons/trophies.png" alt="" width={25} height={25} /> <strong>{formatNumber(member.trophies)}</strong></td>
