...
const player = httpAction(async (ctx, request) => {
  ...
  if (snapshot) {
    await ctx.runMutation(internal.players.recordProfile, snapshot);
  }

  if (items.length) {
    await ctx.scheduler.runAfter(0, internal.ingest.ingestBattleLogItems, {
      items,
      focusTag: tag,
    });
  }
...
const playerSearch = httpAction(async (ctx, request) => {
  ...
  return json(await ctx.runQuery(api.players.search, { query, limit }));
});

const playerHistory = httpAction(async (ctx, request) => {
  const tag = normalizedTag(new URL(request.url).searchParams.get("tag"));
  if (!tag) return json({ error: "INVALID_TAG", message: "Enter a valid Brawl Stars player tag." }, 400);
  return json({ snapshots: await ctx.runQuery(api.players.history, { tag, limit: 180 }) });
});
...
const mapDetail = httpAction(async (ctx, request) => {
  ...
  const meta = await ctx.runQuery(api.stats.getMapStats, {
    mapId,
