...
export const ingestBattleLogItems = internalMutation({
  ...
  handler: async (ctx, args): Promise<{ inserted: number }> => {
    ...
    for (const raw of args.items as BattleLogItem[]) {
      ...
      await ctx.db.insert("seenBattles", {
        dedupeKey,
        mapId,
        mode,
...
export const ingestFromPlayerTag = internalAction({
  ...
  handler: async (ctx, args): Promise<{ inserted: number }> => {
    ...
    return await ctx.runMutation(internal.ingest.ingestBattleLogItems, {
      items,
      focusTag: tag,
    });
  },
...
export const seedFromRankings = internalAction({
  ...
  handler: async (ctx): Promise<{ processed: number }> => {
    ...
    const cursor = await ctx.runQuery(internal.stats.getCursor, { key: "rankings-global-players" });
    ...
    for (const tag of slice) {
      try {
        await ctx.runAction(internal.ingest.ingestFromPlayerTag, { tag });
        processed += 1;
      } catch (error) {
    ...
    await ctx.runMutation(internal.stats.setCursor, {
...
export const ingestBattleLog = action({
  ...
  returns: v.object({ inserted: v.number() }),
  handler: async (ctx, args): Promise<{ inserted: number }> => {
    return await ctx.runMutation(internal.ingest.ingestBattleLogItems, args);
  },
});
