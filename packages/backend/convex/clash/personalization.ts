...
async function touchAccount(ctx: MutationCtx, accountId: Id<"clashPersonalAccounts">, now: number) {
  await ctx.db.patch(accountId, { updatedAt: now });
}

async function upsertProfile(ctx: MutationCtx, accountId: Id<"clashPersonalAccounts">, profile: ProfileInput, now: number) {
  const tag = normalizeTag(profile.tag);
  const existing = await ctx.db
    .query("clashPersonalProfiles")
  ...
  const rows = await ctx.db.query("clashPersonalProfiles").withIndex("by_account_id", (q) => q.eq("accountId", accountId)).take(MAX_PROFILES);
  if (rows.length >= MAX_PROFILES) throw new Error(`You can track up to ${MAX_PROFILES} profiles.`);
  await ctx.db.insert("clashPersonalProfiles", { accountId, kind: profile.kind, tag, ...values, isDefault: false, createdAt: now });
}

async function upsertRecent(ctx: MutationCtx, accountId: Id<"clashPersonalAccounts">, recent: RecentInput) {
  const tag = normalizeTag(recent.tag);
  const existing = await ctx.db
    .query("clashPersonalRecents")
  ...
  else await ctx.db.insert("clashPersonalRecents", { accountId, kind: recent.kind, tag, ...values });
...
async function readState(ctx: QueryCtx | MutationCtx, accountId: Id<"clashPersonalAccounts">) {
  const account = await ctx.db.get(accountId);
  if (!account) throw new Error("Personalization account not found.");
  const [profiles, recents, devices] = await Promise.all([
    ctx.db.query("clashPersonalProfiles").withIndex("by_account_id", (q) => q.eq("accountId", accountId)).take(MAX_PROFILES),
    ctx.db.query("clashPersonalRecents").withIndex("by_account_id", (q) => q.eq("accountId", accountId)).take(MAX_RECENTS),
    ctx.db.query("clashPersonalDevices").withIndex("by_account_id", (q) => q.eq("accountId", accountId)).take(MAX_DEVICES),
  ]);
  return {
    accountId,
    preferences: {
...
export const ensureAccount = mutation({
  ...
  returns: v.id("clashPersonalAccounts"),
  handler: async (ctx, args) => {
    const now = Date.now();
    let device = await deviceForSecret(ctx, args.deviceSecret);
    if (!device) {
      const accountId = await ctx.db.insert("clashPersonalAccounts", {
        createdAt: now,
        updatedAt: now,
        ...args.importedPreferences,
      });
      const secretHash = await hashCapability(args.deviceSecret);
      const deviceId = await ctx.db.insert("clashPersonalDevices", {
        accountId,
        secretHash,
...
export const removeProfile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const observation = await ctx.db
      .query("clashPersonalObservations")
      .withIndex("by_account_id_and_kind_and_tag", (q) => q.eq("accountId", device.accountId).eq("kind", args.kind).eq("tag", tag))
      .unique();
...
export const setDefaultProfile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const profiles = await ctx.db.query("clashPersonalProfiles").withIndex("by_account_id", (q) => q.eq("accountId", device.accountId)).take(MAX_PROFILES);
...
export const clearRecents = mutation({
  ...
  handler: async (ctx, args) => {
    const device = await requireDevice(ctx, args.deviceSecret);
    const recents = await ctx.db.query("clashPersonalRecents").withIndex("by_account_id", (q) => q.eq("accountId", device.accountId)).take(MAX_RECENTS);
...
export const observeProfile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    else await ctx.db.insert("clashPersonalObservations", { accountId: device.accountId, kind: args.kind, tag, ...values });
    return alerts;
  },
});
...
export const createPairingCode = mutation({
  ...
  handler: async (ctx, args) => {
    const device = await requireDevice(ctx, args.deviceSecret);
    const now = Date.now();
    const existing = await ctx.db.query("clashPersonalPairingCodes").withIndex("by_account_id", (q) => q.eq("accountId", device.accountId)).take(6);
    ...
    await ctx.db.insert("clashPersonalPairingCodes", { accountId: device.accountId, codeHash, createdAt: now, expiresAt });
    return { expiresAt };
  },
});

async function mergeAccount(ctx: MutationCtx, sourceId: Id<"clashPersonalAccounts">, targetId: Id<"clashPersonalAccounts">, removeSource: boolean) {
  if (sourceId === targetId) return;
  const sourceProfiles = await ctx.db.query("clashPersonalProfiles").withIndex("by_account_id", (q) => q.eq("accountId", sourceId)).take(MAX_PROFILES);
  const sourceRecents = await ctx.db.query("clashPersonalRecents").withIndex("by_account_id", (q) => q.eq("accountId", sourceId)).take(MAX_RECENTS);
  const sourceObservations = await ctx.db.query("clashPersonalObservations").withIndex("by_account_id", (q) => q.eq("accountId", sourceId)).take(MAX_PROFILES);
  ...
  for (const observation of sourceObservations) {
    ...
    if (!existing) await ctx.db.insert("clashPersonalObservations", { accountId: targetId, kind: observation.kind, tag: observation.tag, ...values });
...
export const redeemPairingCode = mutation({
  ...
  handler: async (ctx, args) => {
    const device = await requireDevice(ctx, args.deviceSecret);
    const codeHash = await hashCapability(args.codeSecret);
    const pairing = await ctx.db.query("clashPersonalPairingCodes").withIndex("by_code_hash", (q) => q.eq("codeHash", codeHash)).unique();
    if (!pairing || pairing.expiresAt < Date.now()) throw new Error("That pairing code is invalid or has expired.");
    const devices = await ctx.db.query("clashPersonalDevices").withIndex("by_account_id", (q) => q.eq("accountId", pairing.accountId)).take(MAX_DEVICES);
    ...
    const sourceDevices = previousAccountId === pairing.accountId
      ? []
      : await ctx.db.query("clashPersonalDevices").withIndex("by_account_id", (q) => q.eq("accountId", previousAccountId)).take(2);
    ...
    if (previousAccountId !== pairing.accountId && removeSource) {
      const remaining = await ctx.db.query("clashPersonalDevices").withIndex("by_account_id", (q) => q.eq("accountId", previousAccountId)).take(1);
      if (!remaining.length) {
...
export const clearAccount = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const [profiles, recents, observations, codes, devices] = await Promise.all([
      ctx.db.query("clashPersonalProfiles").withIndex("by_account_id", (q) => q.eq("accountId", accountId)).take(MAX_PROFILES),
      ctx.db.query("clashPersonalRecents").withIndex("by_account_id", (q) => q.eq("accountId", accountId)).take(MAX_RECENTS),
      ctx.db.query("clashPersonalObservations").withIndex("by_account_id", (q) => q.eq("accountId", accountId)).take(MAX_PROFILES),
      ctx.db.query("clashPersonalPairingCodes").withIndex("by_account_id", (q) => q.eq("accountId", accountId)).take(6),
      ctx.db.query("clashPersonalDevices").withIndex("by_account_id", (q) => q.eq("accountId", accountId)).take(MAX_DEVICES),
    ]);
